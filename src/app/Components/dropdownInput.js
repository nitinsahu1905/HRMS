...
const DropdownInput = ({ mainText, Data ,onSelect,onEnter}) => {
  ...
  return (
    <div>
      ...
      <ul
        id="dropdown-example"
        className={`py-2 absolute bg-white space-y-2 ${isDropdownOpen ? "" : "hidden"}`}
      >
        {Data.map((field) => (
          <li key={field} >
            <div className="w-full cursor-pointer p-3 text-black transition duration-75 rounded-lg group hover:bg-[#f9f9f9] group hover:text-[#121f47] dark:hover:bg-primary-blue" onClick={() => handleSelect(field)}>
              {field}
            </div>
            <div>
